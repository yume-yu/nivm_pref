...
{'FileType'},
{
  ...
  callback = function()
    ...
    vim.api.nvim_buf_set_keymap(0, "n", "<CR>", "<Cmd>call ddu#ui#do_action('itemAction')<CR>", opts)
    -- Spaceでそのitemの選択/解除
    vim.api.nvim_buf_set_keymap(0, "n", "<Space>", "<Cmd>call ddu#ui#do_action('toggleSelectItem')<CR>", opts)
    vim.api.nvim_buf_set_keymap(0, "n", "d", "<Cmd>call ddu#ui#do_action('itemAction', {'name': 'delete'})<CR>", opts)
    -- iで絞り込みバッファの展開
    vim.api.nvim_buf_set_keymap(0, "n", "i", "<Cmd>call ddu#ui#do_action('openFilterWindow')<CR>", opts)
    -- qでバッファを閉じる
    vim.api.nvim_buf_set_keymap(0, "n", "q", "<Cmd>call ddu#ui#do_action('quit')<CR>", opts)
    -- cでアクション選択
    vim.api.nvim_buf_set_keymap(0, "n", "c", "<Cmd>call ddu#ui#do_action('chooseAction')<CR>", opts)
  end
}
)
...
{'FileType'},
{
  ...
  callback = function()
    local opts = { noremap = true, silent = true }
    -- Enterでそのitemに定義されたアクション(ex. 開く など)
    vim.api.nvim_buf_set_keymap(0, "i", "<CR>", "<esc><Cmd>call ddu#ui#do_action('closeFilterWindow')<CR>", opts)
    vim.api.nvim_buf_set_keymap(0, "n", "<CR>", "<Cmd>call ddu#ui#do_action('closeFilterWindow')<CR>", opts)
    -- qでバッファを閉じる
    vim.api.nvim_buf_set_keymap(0, "n", "q", "<Cmd>call ddu#ui#do_action('closeFilterWindow')<CR>", opts)
  end
}
)
...
vim.api.nvim_set_keymap("n", "<Leader>b", "<Cmd>Ddu buffer -name=buffer<CR>", opts)
