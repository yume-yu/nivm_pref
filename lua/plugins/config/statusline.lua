local colors = require 'plugins/config/statusline-colors'
--local lsp = require 'feline.providers.lsp'
require("plugins/config/statusline-colors")
...
local function get_skk_mode()
  ...
  if mode == "" then return "" end -- 通常のローマ字入力時は空文字が返ることがある
...
-- これをfeline.setup()のcomponentsテーブルの好きな場所に挿入する
-- 例: components.active[1] の末尾など
...
local comps = {
  ...
  skkeleton = {
    ...
    update = {
      "ModeChanged",
      "User",        -- 全てのUserイベントを拾う（少し広すぎるが確実）
      "CursorMovedI" -- インサートモードでのカーソル移動時もチェック（念の為）
    }
  }
}
